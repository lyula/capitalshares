...
function companies_insert(&$error_message = '') {
	global $Translation;
	...
	$data = [
		'image' => Request::fileUpload('image', [
			...
			'success' => function($name, $selected_id) {
				Thumbnail::create($name, getThumbnailSpecs('companies', 'image', 'tv'));
				Thumbnail::create($name, getThumbnailSpecs('companies', 'image', 'dv'));
			},
			'failure' => function($selected_id, $fileRemoved) {
				if(!strlen(Request::val('SelectedID'))) return '';

				/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
				return existing_value('companies', 'image', Request::val('SelectedID'));
			},
		]),
		'name' => Request::val('name', ''),
		'share_price' => Request::val('share_price', '0.00'),
		'percentage' => Request::val('percentage', ''),
	];
	...
	update_calc_fields('companies', $recID, calculated_fields()['companies']);
	...
	set_record_owner('companies', $recID, $recordOwner);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) companies_copy_children($recID, Request::val('SelectedID'));
...
function companies_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('companies', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `investments` WHERE `company`='" . makeSafe($id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'investments', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'companies_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'companies_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
...
function companies_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('companies', $selected_id, 'edit')) return false;
	...
	$data = [
		'image' => Request::fileUpload('image', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('companies', 'image', $selected_id);
			},
		]),
		'name' => Request::val('name', ''),
		'share_price' => Request::val('share_price', ''),
		'percentage' => Request::val('percentage', ''),
	];

	// get existing values
	$old_data = getRecord('companies', $selected_id);
	...
	if(function_exists('companies_before_update')) {
		...
		if(!companies_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('companies', $data['selectedID'], calculated_fields()['companies']);
	...
	set_record_owner('companies', $selected_id);
}

function companies_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		if(!check_record_permission('companies', $selected_id, 'view'))
			return $Translation['tableAccessDenied'];

		// can edit?
		$AllowUpdate = check_record_permission('companies', $selected_id, 'edit');

		// can delete?
		$AllowDelete = check_record_permission('companies', $selected_id, 'delete');
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#share_price').replaceWith('<div class=\"form-control-static\" id=\"share_price\">' + (jQuery('#share_price').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#percentage').replaceWith('<div class=\"form-control-static\" id=\"percentage\">' + (jQuery('#percentage').val() || '') + '</div>');\n";
	...
	$templateCode = str_replace('<%%UPLOADFILE(share_price)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(percentage)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(share_price)%%>', safe_html($urow['share_price']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(share_price)%%>', html_attr($row['share_price']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(share_price)%%>', urlencode($urow['share_price']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(percentage)%%>', safe_html($urow['percentage']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(percentage)%%>', html_attr($row['percentage']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(percentage)%%>', urlencode($urow['percentage']), $templateCode);
	} else {
		...
		$templateCode = str_replace('<%%VALUE(share_price)%%>', '0.00', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(share_price)%%>', urlencode('0.00'), $templateCode);
		$templateCode = str_replace('<%%VALUE(percentage)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(percentage)%%>', urlencode(''), $templateCode);
	}
	...
	$templateCode .= loadView('companies-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);
