<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'users' => "`users`.`id` as 'id', `users`.`first_name` as 'first_name', `users`.`last_name` as 'last_name', `users`.`id_number` as 'id_number', `users`.`username` as 'username', `users`.`email` as 'email', `users`.`password` as 'password', if(`users`.`date_joined`,date_format(`users`.`date_joined`,'%d/%m/%Y %H:%i'),'') as 'date_joined', `users`.`refferal_code` as 'refferal_code', `users`.`reffered_by` as 'reffered_by', `users`.`ref_paid` as 'ref_paid'",
			'deposits' => "`deposits`.`id` as 'id', IF(    CHAR_LENGTH(`users1`.`username`), CONCAT_WS('',   `users1`.`username`), '') as 'user_id', `deposits`.`amount` as 'amount', `deposits`.`phone_number` as 'phone_number', if(`deposits`.`date`,date_format(`deposits`.`date`,'%d/%m/%Y'),'') as 'date', `deposits`.`status` as 'status', `deposits`.`mpesa_code` as 'mpesa_code', `deposits`.`payment_refference` as 'payment_refference'",
			'withdrawals' => "`withdrawals`.`id` as 'id', IF(    CHAR_LENGTH(`users1`.`username`), CONCAT_WS('',   `users1`.`username`), '') as 'user_id', `withdrawals`.`amount` as 'amount', `withdrawals`.`phone_number` as 'phone_number', if(`withdrawals`.`date`,date_format(`withdrawals`.`date`,'%d/%m/%Y'),'') as 'date', `withdrawals`.`status` as 'status', `withdrawals`.`mpesa_code` as 'mpesa_code', `withdrawals`.`payment_refference` as 'payment_refference'",
			'wallets' => "`wallets`.`id` as 'id', IF(    CHAR_LENGTH(`users1`.`username`), CONCAT_WS('',   `users1`.`username`), '') as 'user_id', `wallets`.`wallet_type` as 'wallet_type', `wallets`.`available_amount` as 'available_amount'",
			'companies' => "`companies`.`id` as 'id', `companies`.`image` as 'image', `companies`.`name` as 'name', `companies`.`share_price` as 'share_price', `companies`.`percentage` as 'percentage'",
			'investments' => "`investments`.`id` as 'id', IF(    CHAR_LENGTH(`users1`.`username`), CONCAT_WS('',   `users1`.`username`), '') as 'user_id', IF(    CHAR_LENGTH(`companies1`.`name`), CONCAT_WS('',   `companies1`.`name`), '') as 'company', `investments`.`amount` as 'amount', `investments`.`pay_per_day` as 'pay_per_day', if(`investments`.`date_invested`,date_format(`investments`.`date_invested`,'%d/%m/%Y %H:%i'),'') as 'date_invested', `investments`.`shares_sold` as 'shares_sold'",
			'affiliates' => "`affiliates`.`id` as 'id', `affiliates`.`total_affiliate_bonus` as 'total_affiliate_bonus'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'users' => "`users` ",
			'deposits' => "`deposits` LEFT JOIN `users` as users1 ON `users1`.`id`=`deposits`.`user_id` ",
			'withdrawals' => "`withdrawals` LEFT JOIN `users` as users1 ON `users1`.`id`=`withdrawals`.`user_id` ",
			'wallets' => "`wallets` LEFT JOIN `users` as users1 ON `users1`.`id`=`wallets`.`user_id` ",
			'companies' => "`companies` ",
			'investments' => "`investments` LEFT JOIN `users` as users1 ON `users1`.`id`=`investments`.`user_id` LEFT JOIN `companies` as companies1 ON `companies1`.`id`=`investments`.`company` ",
