	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'users' => ['Users', '', 'table.gif', 'None'],
			'deposits' => ['Deposits', '', 'table.gif', 'None'],
			'withdrawals' => ['Withdrawals', '', 'table.gif', 'None'],
			'wallets' => ['Wallets', '', 'table.gif', 'None'],
			'companies' => ['Companies', '', 'table.gif', 'None'],
			'investments' => ['Investments', '', 'table.gif', 'None'],
			'affiliates' => ['Affiliates', '', 'table.gif', 'None'],
		];
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		if($tableName=='companies' && $fieldName=='image' && $view=='tv')
			return ['width'=>50, 'height'=>50, 'identifier'=>'_tv'];
		elseif($tableName=='companies' && $fieldName=='image' && $view=='dv')
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'deposits' => [
				'user_id' => 'SELECT `users`.`id`, `users`.`username` FROM `users` ORDER BY 2',
			],
			'withdrawals' => [
				'user_id' => 'SELECT `users`.`id`, `users`.`username` FROM `users` ORDER BY 2',
			],
			...
			'investments' => [
				'user_id' => 'SELECT `users`.`id`, `users`.`username` FROM `users` ORDER BY 2',
				'company' => 'SELECT `companies`.`id`, `companies`.`name` FROM `companies` ORDER BY 2',
			],
			'affiliates' => [
			],
		];
